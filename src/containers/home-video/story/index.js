...
const Story = ({ section, paragraph, paragraphTwo, button, imageStyle }) => {
	const storyQueryData = useStaticQuery(graphql`
        query VideoStoryDataQuery {
            homevideodataJson(section: {eq: "video_story_section_contnet"}) {
                id
                title
                subtitle
                path
    ...
    `)
	const storyData = storyQueryData.homevideodataJson;
					...
					<Controller>
						<Scene classToggle="animated" triggerElement="#trigger" triggerHook="onCenter">
							<StoryImgWrap className="rn_surface story-image">
								<StoryImage {...bgImage} backgroundPosition="left" />
							</StoryImgWrap>
						</Scene>
					</Controller>
				</div>
